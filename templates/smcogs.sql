CREATE TABLE "{{DATABASE}}"."smcogs" (
    smcog_id	serial NOT NULL,
    name	text,
    description	text,
    functional_class_id	int4,
    CONSTRAINT smcogs_pkey PRIMARY KEY (smcog_id),
    CONSTRAINT smcogs_functional_class_id_fkey FOREIGN KEY (functional_class_id) REFERENCES "{{DATABASE}}"."functional_classes" (functional_class_id)
);
COMMENT ON TABLE "{{DATABASE}}"."smcogs" IS
  'Secondary Metabolite specific Clusters of Orthologous Groups (smCoGs)';


INSERT INTO "{{DATABASE}}"."smcogs" (name, description, functional_class_id)
