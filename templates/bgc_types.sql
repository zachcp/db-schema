CREATE TABLE "{{DATABASE}}"."bgc_types" (
    ...
    CONSTRAINT bgc_types_parent_id_fkey FOREIGN KEY (parent_id) REFERENCES "{{DATABASE}}"."bgc_types" (bgc_type_id)
);

COMMENT ON TABLE "{{DATABASE}}"."bgc_types" IS
  'Biosynthetic gene cluster types. Basic types according to MIBiG spec.';

--- basic MIBiG types
INSERT INTO "{{DATABASE}}"."bgc_types" (term, description, parent_id)
...
LEFT JOIN "{{DATABASE}}"."bgc_types" f ON val.parent_term = f.term;
